...
data "template_file" "inventory" {
  ...
  vars = {
    public_ip_address_bastion = join("\n", formatlist("bastion ansible_user=admin ansible_host=%s", aws_instance.bastion-server.*.public_ip))
    connection_strings_master = join("\n", formatlist("%s ansible_user=admin ansible_host=%s", aws_instance.k8s-master.*.private_dns, aws_instance.k8s-master.*.public_ip))
    connection_strings_node   = join("\n", formatlist("%s ansible_user=admin ansible_host=%s", aws_instance.k8s-worker.*.private_dns, aws_instance.k8s-worker.*.public_ip))
    list_master               = join("\n", aws_instance.k8s-master.*.private_dns)
    list_node                 = join("\n", aws_instance.k8s-worker.*.private_dns)
    connection_strings_etcd   = join("\n", formatlist("%s ansible_user=admin ansible_host=%s", aws_instance.k8s-etcd.*.private_dns, aws_instance.k8s-etcd.*.public_ip))
